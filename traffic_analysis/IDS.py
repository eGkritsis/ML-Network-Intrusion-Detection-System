...
def unsupervised_flow_detection(captured_flows):
    ...
    max_features = max(len(row) for row in dataset)

    # Pad flows with fewer features with zeros
    for i, flow in enumerate(dataset):
        if len(flow) < max_features:
            dataset[i] += [0.0] * (max_features - len(flow))
    ...
    # Calculate the distance to the nearest core sample for each instance
    ...
    # Detect anomalies in the dataset (label -1 denotes anomalies/outliers)
    anomalies = model.labels_ == -1

    # Generate report
    generate_anomaly_report(captured_flows, anomalies)
